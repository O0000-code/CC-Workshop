...
- `/Applications/Ensemble.app` (current production)
- `/Applications/Ensemble Dev.app` (current test)
- `/Applications/Ensemble Backups/Ensemble-20260506-134612.app` (timestamped backup that the 2026-05-06 install silently created)
- `~/Downloads/Ensemble_1.0.0_aarch64-2.dmg` (a February DMG nobody ever opened)
- `src-tauri/target/release/bundle/macos/Ensemble.app` + DMG (build output, never cleaned)
- `src-tauri/target/debug/bundle/macos/Ensemble Dev.app` + DMG

A Spotlight search for "Ensemble" returned five `.app` / `.dmg` hits and the user could not tell which one was current. The complaint — "保持干净，不然哪个是哪个都不知道" — names exactly this failure mode.
...
- Spotlight indexes them, polluting every future Ensemble search.
...
1. **Overwrite same-name in place.** `rm -rf /Applications/Ensemble.app && cp -R <new> /Applications/Ensemble.app`. No `mv` of the old `.app` to a side path between the `rm` and the `cp`.
2. **One canonical name per artifact role.** Production = `/Applications/Ensemble.app`; test = `/Applications/Ensemble Dev.app`. Names like `Ensemble-2.app`, `Ensemble (1).app`, `Ensemble-YYYYMMDD-HHMMSS.app`, `Ensemble-old.app` are forbidden — even temporarily inside the same task.
...
- Creating `Ensemble Backups/`, `Ensemble Old/`, or any directory whose purpose is "preserve last `.app` before the new one lands"
- Renaming the old `.app` with a timestamp or version-number suffix prior to the overwrite
- Co-existing `Ensemble.app` and `Ensemble (1).app` / `Ensemble-2.app` / `Ensemble-old.app` in any install location
...
- Two artifacts that are genuinely different roles, not backups of each other — `Ensemble.app` + `Ensemble Dev.app` with distinct `CFBundleIdentifier`. Each role independently obeys the rule (one `.app` per role per location).
...
- `cp -R /Applications/Ensemble.app "/Applications/Ensemble Backups/Ensemble-$(date +%Y%m%d-%H%M%S).app" && cp -R new /Applications/Ensemble.app` — the canonical "safety backup" reflex; forbidden.
- Leaving `target/release/bundle/macos/Ensemble.app` populated **after** copying it to `/Applications/Ensemble.app` — same `.app` in two locations, Spotlight returns two hits.
...
- Naming a kept-around old version `Ensemble-old.app`, `Ensemble.bak.app`, `Ensemble-prev.app` — a single such artifact is already an accumulation.
