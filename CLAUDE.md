...
All app data lives in `~/.cc-workshop/`:
...
- **管理与存储**: `~/.cc-workshop/`(data.json + 独立的 skill 目录 / mcp 文件 / claude-md 文件 / rule 文件)
- **重新部署**: Scene 打包 → 写入 Project 的 `.claude/skills/`(symlink)+ `.mcp.json` + 一个 CLAUDE.md + N 个 Rule 文件

`~/.cc-workshop/data.json` 是唯一持久化的 canonical state(由 `DATA_MUTEX` 串行化写入);Skill 目录 / MCP JSON / CLAUDE.md / Rule 内容都以**独立文件**落盘,data.json 只存元数据指针。

### 五个核心实体

| 实体 | 持久位置 | id 形式 | Page |
|---|---|---|---|
| **Skill** | `data.json::skillMetadata` HashMap + `~/.cc-workshop/skills/<name>/` | `id == sourcePath`(**不变式**)| `/skills` |
| **MCP Server** | `data.json::mcpMetadata` HashMap + `~/.cc-workshop/mcps/<name>.json` | `id == sourcePath`(**不变式**)| `/mcp-servers` |
| **CLAUDE.md** | `data.json::claudeMdFiles` 数组 + `~/.cc-workshop/claude-md/{uuid}/CLAUDE.md` | UUID | `/claude-md` |
| **Rule** | `data.json::rules` 数组 + `~/.cc-workshop/rules/{uuid}/<filename>.md` | UUID(+ 持久 `filename` 字段)| `/rules` |
...
- **local**: 实体目录 / JSON
- **plugin**: `~/.cc-workshop/` 内是 **symlink**,目标在 `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/...`。删除 plugin 即失效
